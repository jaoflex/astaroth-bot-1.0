	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝐟𝐨𝐢 𝐭𝐚𝐫𝐝𝐞 𝐊𝐊𝐊𝐊𝐊𝐊𝐊𝐊𝐊 @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
           ...
           return false
			...
			switch(command) {
			                ...
			                const uangku = checkATMuser(sender)
                                        await costum(help(pushname, prefix, botName, ownerName, reqXp, uangku), text, tescuk,{quoted: { key:  { participant: `0@s.whatsapp.net`, ...{}}, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": 'Sua menzagem', 'jpegThumbnail': fs.readFileSync('./sticker/botlogo.png')}}}})
                                        break
                                case '18+menu':
                                        if (!isRegister) return reply(mess.only.daftarB)
